...
void SimpleImageHistogram::ComputeFromBaseData( BaseData* src )
{
...

  {
    ...
    unsigned int num = 1;
    for (unsigned int r = 0; r < std::min(source->GetDimension(), 3u); r++) {
    ...
    ImageRegionAccessor accessor(source);
    ImageAccessLock lock(&accessor);

    for (unsigned int timestep = 0; timestep < source->GetTimeSteps(); timestep++) {
      const void *src = accessor.getData(0);
      int steps = num;
      do {
        int value = 0;
